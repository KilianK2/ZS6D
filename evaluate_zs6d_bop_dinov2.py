...
if __name__ == "__main__":
    ...
    # adjust for ycbv
    parser.add_argument('--config_file', default="./zs6d_configs/bop_eval_configs/cfg_ycbv_inference_bop_dinov2.json")
            ...
            if bbox_gt != [-1, -1, -1, -1]:
                ...
                try:
                    ...
                    with torch.no_grad():
                        ...
                        img_data.descs.append(desc_dino.squeeze(0).squeeze(0).detach().cpu())
                ...
                for matched_template in matched_templates:
                    ...
                    try:
                        with torch.no_grad():
                            ...
                            mask_template ="empty"
                            points1, points2, crop_pil, template_pil = extractor.find_correspondences_kmeans_dinoV2_v13(mask_crop, mask_template, cropped_image, cropped_pil,
