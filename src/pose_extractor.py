...
class PoseViTExtractor(ViTExtractor):
    ...
    def find_correspondences_fastkmeans_sd_dino(self, image_size_sd, model_sd, aug_sd, num_patches, input_image,
        ...
        start_time_saliency = time.time()
        # extracting saliency maps for each image
        saliency_map1 = self.extract_saliency_maps(input_pil.to(self.device))[0]
        saliency_map2 = self.extract_saliency_maps(template_pil.to(self.device))[0]
        end_time_saliency = time.time()
        elapsed_saliencey = end_time_saliency - start_time_saliency
        ...
        # remove best buddies where at least one descriptor is marked bg by saliency mask.
        fg_mask2_new_coors = nn_2[fg_mask2]
        fg_mask2_mask_new_coors = torch.zeros(num_patches1 * num_patches1, dtype=torch.bool, device=self.device)
        fg_mask2_mask_new_coors[fg_mask2_new_coors] = True
        bbs_mask = torch.bitwise_and(bbs_mask, fg_mask1)
        bbs_mask = torch.bitwise_and(bbs_mask, fg_mask2_mask_new_coors)
    ...
    def find_correspondences_fastkmeans_sd_dino_v4(self, model_sd, aug_sd, num_patches, pil_img1, pil_img2,
                                                   num_pairs: int = 10, load_size: int = 840,
    ...
    def find_correspondences_fastkmeans_sd_dino_v7(self, image_size_sd, model_sd, aug_sd, num_patches, input_image,
                                                   input_pil, template_image, template_pil, num_pairs: int = 10,
                                                   load_size: int = 840,
                                                   layer: int = 11, facet: str = 'token', bin: bool = False,
                                                   thresh: float = 0.05) -> Tuple[
    ...
    # nearst neighbor for correspondences
