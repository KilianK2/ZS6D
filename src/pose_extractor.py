...
class PoseViTExtractor(extractor.ViTExtractor):
    ...
    def find_correspondences_fastkmeans_sd_dino_v4(self, pil_img1, pil_img2, num_pairs: int = 10, load_size: int = 840,
    ...
    def find_correspondences_fastkmeans_sd_dino_v6(self, pil_img1, pil_img2, num_pairs: int = 10, load_size: int = 840,
                                                layer: int = 11, facet: str = 'token', bin: bool = True,
                                                thresh: float = 0.05) -> Tuple[
        List[Tuple[float, float]], List[Tuple[float, float]], Image.Image, Image.Image]:
        ...
        # normalization
        descriptors1_dino = descriptors1_dino / descriptors1_dino.norm(dim=-1, keepdim=True)
        descriptors1_sd = descriptors1_sd / descriptors1_sd.norm(dim=-1, keepdim=True)
        ...
        # normalization
        descriptors2_dino = descriptors2_dino / descriptors2_dino.norm(dim=-1, keepdim=True)
        descriptors2_sd = descriptors2_sd / descriptors2_sd.norm(dim=-1, keepdim=True)
