...
class Fused_ZS6D:
    ...
    def get_pose(self, model, aug, img, obj_id, mask, cam_K, bbox=None):
        try:
            ...
            """Fused Descriptors"""
            desc_sd_dino = self.get_fused_features(model, aug, img_crop_pil)
            ...
            with torch.no_grad():
                if img_crop_pil.size[0] < self.max_crop_size:
                    crop_size = img_crop_pil.size[0]
                else:
                    crop_size = self.max_crop_size

                resize_factor = float(crop_size) / img_crop_pil.size[0]

                points1, points2, crop_pil, template_pil = self.extractor.find_correspondences_fastkmeans(img_crop_pil,
                                                                                                          template,
                                                                                                          num_pairs=20,
                                                                                                          load_size=crop_size)
    ...
    def get_fused_features_old(self, model, aug, img):
        PCA = True
        EDGE_PAD = False
